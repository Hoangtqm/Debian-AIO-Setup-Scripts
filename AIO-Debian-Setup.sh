...
do_tlauncher() {
    ...
    # ── Tải và cài TLauncher .deb ────────────────────────────
    log_step "  Tải TLauncher installer (.deb)"
    local tmp; tmp=$(mktemp -d)
    local deb="${tmp}/tlauncher-installer.deb"

    wget -q --show-progress \
        "https://dl1.tlauncher.org/f.php?f=files%2Ftlauncher-linux-installer.deb" \
        -O "$deb"

    log_step "  Cài TLauncher từ .deb"
    sudo $APT install $APT_OPTS "$deb"
    rm -rf "$tmp"

    log_ok "TLauncher đã cài xong!"
    log_warn "Mở TLauncher từ App Menu — chọn đúng Java 17 trong Settings của TLauncher"
    echo ""
    echo -e "  ${GRAY}  Nếu TLauncher không tự chọn Java 17, vào:${RESET}"
    echo -e "  ${WHITE}  TLauncher → Settings → Java → chọn Java 17.0.12${RESET}"
}
